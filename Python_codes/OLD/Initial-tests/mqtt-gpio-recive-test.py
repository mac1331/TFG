...
# mode=GPIO.getmode()
# print("mode is sms ",mode)
GPIO.setup(4,GPIO.OUT)
# # GPIO.output(4,1)
# print("value is ",GPIO.OUT(4))
...
messages=[]
sub_topic="intstatus"
client= mqtt.Client("GPIO-client-003")  #create client object client1.on_publish = on_publish                          #assign function to callback client1.connect(broker,port)                                 #establish connection client1.publish("house/bulb1","on")  
######
...
while True:
    client.loop(0.001)
#     client.publish("ledstatus",GPIO.input(4))
    time.sleep(0.01)
    if len(messages)>0:
        m=messages.pop(0)
        print("received ",m)
