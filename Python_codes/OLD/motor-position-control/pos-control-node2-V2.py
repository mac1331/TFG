...
GPIO.setup(16, GPIO.OUT)    # set GPIO 25 as output for the PWM signal
GPIO.output(20,GPIO.HIGH)
GPIO.output(16,GPIO.LOW)
GPIO.setup(26, GPIO.OUT)    # set GPIO 25 as output for the PWM signal
D2A = GPIO.PWM(26, 10000)    # create object D2A for PWM on port 25 at 1KHz
...
#         client.loop(100)
        ...
        error = abs(desiredpos - realpos)
        while abs(error) != 0:
            error = desiredpos - realpos
            time.sleep(0.2)
            dutycycle = abs(error*p)
            if dutycycle >= 100:
                dutycycle = 100
            print (error)
            if error < 0:
